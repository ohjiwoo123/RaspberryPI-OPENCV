...
int main(int, char**)
{
    ...
    while(1)
    {
        ...
        int makeFolder;
        sprintf(filePath, "%s/%s",path,fBUF);
        makeFolder=mkdir(filePath,0700);
        sprintf(filePath, "%s/%s/%s",path,fBUF,tBUF);
        // 카메라에서 매 프레임마다 이미지 읽기 
        writer.open(filePath, VideoWriter::fourcc('D','I','V','X'),
        videoFPS, Size(videoWidth, videoHeight), true);
        getTime(LOG_TIME);
        if(makeFolder==0){
            sprintf(buff,"%s명으로 폴더가 생성되었습니다.\n",fBUF);
            WRByte=write(fd,buff,strlen(buff));
        }
    ...
    // 용량이 부족하면 가장 오래된 폴더를 삭제한다. 
        if(AvailableMemory()<60)
        {
            ...
            printf("용량이 부족하여 가장 오래된 폴더 %s를 삭제합니다\n",rmdirName);
            rmdirs(rmdirName,1);
